...
fn sample_uniform_disk(radius: f32) -> Vec2 {
    let vec2 = Vec2::from_angle(rand::random::<f32>()*std::f32::consts::TAU);
    vec2 * radius*(rand::random::<f32>()).sqrt()
}
    ...
    pub fn compute_trajectories(&mut self, root: usize, particle_per_leaf: usize) {
        ...
        match &self.node_info[root].children[..] {
            ...
            &[m_child, s_child] => {
                ...
                self.project_particles(root, m_child, offset_direction * self.node_props[m_child].radius);
                self.project_particles(root, s_child, -offset_direction * self.node_props[s_child].radius);
            }
