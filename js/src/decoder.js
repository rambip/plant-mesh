...
class TreeMeshDecoder {
    ...
    decode(jsonInput) {
        ...
        for (const [name, buf] of Object.entries(buffers)) {
            ...
            const decoded = this._decodeBuffer(
                buf.data,
                buf.k,
                offset,
                buf.length,
            ...
            if (buf.length !== undefined && decoded.length !== buf.length) {
                throw new Error(
                    `Buffer "${name}": expected ${buf.length} values, got ${decoded.length}. ` +
        ...
        try {
            ...
            const colors = outputs.colors
                ? this._eval(outputs.colors, scope)
                : null;
            const triangles = this._eval(outputs.triangles, scope);

            if (vertices.length !== normals.length) {
                throw new Error(
                    `Vertex/normal count mismatch: ${vertices.length} vertices vs ${normals.length} normals.`,
                );
            }
            ...
            if (triangles.length % 3 !== 0) {
                throw new Error(
                    `Triangle buffer length ${triangles.length} is not a multiple of 3.`,
                );
            }
            ...
            const colorBuffer = colors
                ? this._rgbaToInterleaved(colors)
                : new Float32Array(0);
    ...
    _riceDecode(byteArray, k, length = undefined) {
        ...
        while (
            (byteIndex < byteArray.length || bitCount > 0) &&
            result.length < limit
        ) {
            let quotient = 0;
            while (readBit() === 1) {
            ...
            const signed =
                (zigzag & 1) === 0 ? zigzag >> 1 : -(zigzag >> 1) - 1;
            result.push(signed);
        }
    ...
    _eval(expr, scope) {
        ...
        if (Array.isArray(expr)) {
            throw new Error(
                `Expected expression object, got array: ${JSON.stringify(expr).slice(0, 100)}`,
            );
        }

        if (typeof expr !== "object" || expr === null) {
            throw new Error(
                `Invalid expression: ${expr}, expected object with 'op' field`,
            );
        }
        ...
        if (!op) {
            throw new Error(
                `Expression missing 'op' field: ${JSON.stringify(expr).slice(0, 200)}`,
            );
        }
        ...
        if (!Array.isArray(expr.args)) {
            throw new Error(
                `Operator '${op}' 'args' must be an array, got: ${typeof expr.args}`,
            );
        }
    ...
    _vec4(x, y, z, w) {
        if (
            ...
            throw new Error(
                `vec4 component length mismatch: x=${x.length}, y=${y.length}, z=${z.length}, w=${w.length}. ` +
    ...
    _interleave(buffers) {
        ...
        for (const buf of buffers) {
            if (buf.length !== N) {
                throw new Error(
                    `interleave: all buffers must have equal length, got ${N} and ${buf.length}`,
                );
            }
        }
        if (
