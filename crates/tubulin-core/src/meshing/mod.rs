use glam::{FloatExt, Mat3, Quat, Vec2, Vec3};
use rand::prelude::Distribution;
use rand::Rng;
use serde::{Deserialize, Serialize};

use crate::growing::TreeSkeleton;
pub use crate::meshing::algorithms::SplineIndex;
use crate::meshing::algorithms::{
    convex_hull_graham, extended_catmull_spline, mesh_between_contours,
};
...
pub use particles::StrandsConfig as StrandsConfigAlias; // Avoid name collision if any
    ...
    fn compute_each_branch_recursive(
        ...
        match children.len() {
            ...
            2 => {
                ...
                let m_branch_length =
                    (self.tree.position(m_child) - self.tree.position(root)).length();
                let s_branch_length =
                    (self.tree.position(s_child) - self.tree.position(root)).length();
