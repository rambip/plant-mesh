  ...
  class TreeMeshDecoder {
    ...
    _eval(expr, scope) {
      ...
      try {
        ...
        throw new Error(`Eval failed for op='${op}' argTypes=[${argTypes.join(", ")}] expr=${JSON.stringify(expr).slice(0, 220)}: ${base}`);
      }
    }
    _assertArgCount(op, args, expected) {
      if (args.length !== expected) {
        throw new Error(`Operator '${op}' expected ${expected} args, got ${args.length}`);
      }
    }
    _describeValue(value) {
      ...
      if (Array.isArray(value)) {
        if (value.length === 0)
          return "Array(len=0)";
        const first = value[0];
        if (typeof first === "object" && first !== null && "x" in first && "y" in first && "z" in first) {
    ...
    _cumsum(buffer) {
      if (!(buffer instanceof Int32Array)) {
        throw new Error(`cumsum expects Int32Array, got ${this._describeValue(buffer)}`);
      }
    ...
    _divp2(buffer, exponent) {
      if (typeof exponent !== "number" || !Number.isInteger(exponent) || exponent < 0) {
        throw new Error(`divp2 exponent must be a non-negative integer, got ${this._describeValue(exponent)}`);
      }
      ...
      if (Array.isArray(buffer)) {
        ...
        if (typeof buffer[0] === "object" && buffer[0] !== null && "x" in buffer[0]) {
          return buffer.map((v) => ({
            x: v.x / divisor,
            y: v.y / divisor,
            z: v.z / divisor,
      ...
      throw new Error(`divp2 expects Int32Array/Float32Array or Vec3/Vec4 array, got ${this._describeValue(buffer)}`);
    }
    _vec3(x, y, z) {
      if (!x || !y || !z || x.length === undefined || y.length === undefined || z.length === undefined) {
        throw new Error(`vec3 expects three array-like buffers, got x=${this._describeValue(x)}, y=${this._describeValue(y)}, z=${this._describeValue(z)}`);
      }
    ...
    _vec4(x, y, z, w) {
      if (!x || !y || !z || !w || x.length === undefined || y.length === undefined || z.length === undefined || w.length === undefined) {
        throw new Error(`vec4 expects four array-like buffers, got x=${this._describeValue(x)}, y=${this._describeValue(y)}, z=${this._describeValue(z)}, w=${this._describeValue(w)}`);
      }
    ...
    _triangle(i, j, k) {
      if (!i || !j || !k || i.length === undefined || j.length === undefined || k.length === undefined) {
        throw new Error(`triangle expects three array-like buffers, got i=${this._describeValue(i)}, j=${this._describeValue(j)}, k=${this._describeValue(k)}`);
      }
...
function init(geometryData) {
  ...
  function showDecodeError(error, inputData) {
    ...
    const outputKeys = inputData && inputData.outputs ? Object.keys(inputData.outputs) : [];
    const bufferCount = inputData && inputData.buffers ? Object.keys(inputData.buffers).length : 0;
...
`);
  ...
  function updateCamera(timeSeconds) {
    const angle = cameraSettings.automaticMode ? 0.5 * timeSeconds : cameraSettings.orbitAngle;
  ...
  renderer.domElement.addEventListener("pointermove", (event) => {
    ...
    cameraSettings.orbitAngle -= 0.0005 * dx * cameraSettings.orbitDistance * cameraSettings.sensibility;
  });
